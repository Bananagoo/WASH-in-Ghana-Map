...
class TitleScreen(BaseScreen):
    ...
    def draw(self, surface: pygame.Surface):
        ...
        surface.blit(title_s, title_s.get_rect(center=(w // 2, 58)))

        sub_s = self._font_sub.render(self._subtitle, True, C.WHITE)
        surface.blit(sub_s, sub_s.get_rect(center=(w // 2, 110)))

        course_s = self._font_xs.render(self._course, True, C.TEAL_LIGHT)
        surface.blit(course_s, course_s.get_rect(center=(w // 2, 148)))

        # ── Tutorial panel ────────────────────────────────────────────────
        panel_w   = 880
        panel_x   = w // 2 - panel_w // 2
        panel_top = HEADER_H + 20
        panel_bot = h - 100
        ...
        img_y = rule_y + 8
        image_rect = pygame.Rect(img_x, img_y, _FRONT_IMG_W, _FRONT_IMG_H)
        ...
        for line in self._tutorial_lines:
            ty = _draw_wrapped_around_image(
                ...
                text_x, ty, full_text_w, side_text_w, image_rect,
                line_spacing=5,
            )
            ty += 8
        ...
        surface.blit(hint_s, hint_s.get_rect(center=(w // 2, h - 14)))
...
def _draw_wrapped_around_image(surface, text, font, colour, x, y,
    ...
    def width_for_line(line_y):
        line_bottom = line_y + font.get_height()
        if image_rect.width and line_bottom > image_rect.top and line_y < image_rect.bottom + 8:
            return side_w
        return full_w

    for word in words:
        max_w = width_for_line(y)
